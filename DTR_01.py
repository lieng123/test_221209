...
path_test3 = 'test3.xlsx'
def read_xlsx(excel_path,sheet_num=0):
...
def read_xlsx2(excel_path,sheet_num=0):
    global pdis 
...
read_xlsx(path2)
read_xlsx2(path_test3)


list_pt = [] 
for i in range(len(pdis)):
    list_pt.append([test_label_name[i],pdis[i]])
print(list_pt)
...
for i in range(len(skc_mid)): 
    skc_mid[i] = re.sub("[\(\)]", "", str(skc_mid[i]))
    skc_mid[i] = skc_mid[i].split(',') 
    skc_mid[i] = list(map(int,skc_mid[i]))
print(skc_mid)
#(472,407)
#(236,203) 
#skc_mid = list(map(int,skc_mid)) 

skc_dot = ((math.pow((skc_mid[1][0]-236),2)+math.pow((skc_mid[1][1]-203),2))) / (472*407) 
...
for i in range(len(skc_answer)):
    for j in range(len(pdis)):
        if(skc_answer[i]==test_label_name[j]):
            sim_label.append(skc_answer[i])
print(sim_label) #두 xlsx 같은 label




for i in range(len(skc_answer)): 
    if(skc_answer[i] not in skc_answer_no_same):
        skc_answer_no_same.append(skc_answer[i]) 

for i in range(len(skc_answer_no_same)):
    skc_answer_num.append(0)
for i in range(len(skc_answer_no_same)):
    for j in range(len(skc_answer)):
        if(skc_answer[j]==skc_answer_no_same[i]):
            skc_answer_num[i]+=1
...
for i in range(len(file_name_list_skc)):
    skc_path = path_skc+file_name_list_skc[i] 
    ...
    for i in range(len(skc_answer)):
        skc_score += (math.pow((skc_mid[i][0]-skc_wid/2),2)+math.pow((skc_mid[i][1]-skc_wid/2),2))*skc_size[i]/skc_wid_hei

    print("skc_score:",skc_score)

    list_pt2 = [] 
    for i in range(len(sim_label)):
        for j in range(len(list_pt)):
            if(list_pt[j][0]==sim_label[i]):
                list_pt2.append(list_pt[j])
    #print(list_pt2)

    score_full = 0
    for i in range(len(list_pt2)):
        score_full += list_pt2[i][1]
